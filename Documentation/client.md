...
The shapeography `client` module hosts the functions `get_shapefiles()` and `get_geojson()` that acts as the client to retrieve shapefiles/GEOJSON files at a specified URL and then manage those files locally.
...
4) refresh (Boolean) - Default=True. When set to True, the branch that hosts the shapefiles files is completely
...
***def get_geojson(url,
    ...
    This function is the client that downloads and locally manages GEOJSON files. 
    This client supports VPN/PROXY connections. 
    ...
    4) refresh (Boolean) - Default=True. When set to True, the branch that hosts the GEOJSON files is completely
       cleaned out and a new set of GEOJSON is downloaded with each run. This is recommended for those using 
       shapeography in automated tasks to account for periodic shapefile/GEOJSON updates on the servers that host the GEOJSONs.
    
    Returns
    -------
    
    GEOJSON to f:{path}/{filename} 
